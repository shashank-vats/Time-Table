...
Schedule* Schedule::Crossover(const Schedule& parent2) const
{
	...
	bool first = rand() % 2 == 0;
	for( int i = 0; i < size && it1 != _classes.end() && it2 != parent2._classes.end(); i++ )
	{
		if( first )
		{
			...
			for( int j = ( *it1 ).first->GetDuration() - 1; j >= 0; j-- )
				n->_slots[ ( *it1 ).second + j ].push_back( ( *it1 ).first );
		}
		else
		{
			...
			// all time-space slots of class are copied
			for( int j = ( *it2 ).first->GetDuration() - 1; j >= 0; j-- )
				n->_slots[ ( *it2 ).second + j ].push_back( ( *it2 ).first );
		}
...
void Algorithm::Start()
{
	...
	while( 1 )
	{
		...
		for( int j = 0; j < _replaceByGeneration; j++ )
		{
			...
			int bbbb = rand() % _chromosomes.size();
			int aaaa = rand() % _chromosomes.size();
			Schedule* p1 = _chromosomes[ aaaa ];
			Schedule* p2 = _chromosomes[ bbbb ];

			int jttt = p2->_classes.size();
			int ittt = p1->_classes.size();
